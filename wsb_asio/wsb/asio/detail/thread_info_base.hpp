#ifndef WSB_ASIO_DETAIL_THREAD_INFO_BASE_HPP
#define WSB_ASIO_DETAIL_THREAD_INFO_BASE_HPP
...
class thread_info_base : noncopyable {
public:
    struct default_tag {
        enum {mem_index = 0 };
    };
    ...
    static void* allocate(Purpose, thread_info_base* this_thread, std::size_t size)
    {
        ...
        if (this_thread && this_thread->reusable_memory_[Purpose::mem_index]) {
            ...
            ::operator delete(pointer); // 不够，则删除，重新分配
        }
        void* const pointer = ::operator new(chunks * chunk_size + 1);
        unsigned char* const mem = static_cast<unsigned char*>(pointer);
        // 当size > 127 * 4 = 512时，mem[size] = 0，这将导致每次分配时，都需要将原先的删除
